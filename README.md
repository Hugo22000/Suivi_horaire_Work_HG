...
1. Copier `.env.example` en `.env` et renseigner :
   - `POSTGRES_PRISMA_URL` / `POSTGRES_URL_NON_POOLING` : connexion vers une base PostgreSQL (locale ou distante).
...
3. Dans l'onglet **Storage** du projet Vercel, créer/lier une base **Postgres** (Neon) : cela ajoute automatiquement les variables `POSTGRES_PRISMA_URL` et `POSTGRES_URL_NON_POOLING`.

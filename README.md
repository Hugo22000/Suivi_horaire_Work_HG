...
3. Dans l'onglet **Storage** du projet Vercel, créer/lier une base **Postgres (Neon)** : cela ajoute automatiquement les variables `DATABASE_URL` (connexion poolée, utilisée par l'application) et `DATABASE_URL_UNPOOLED` (connexion directe, utilisée pour les migrations). **Important** : dans **Settings → Environment Variables**, vérifiez que ces deux variables sont bien cochées pour l'environnement **Production** (pas seulement Preview/Development) — sinon le build échoue avec une erreur `Environment variable not found`.
